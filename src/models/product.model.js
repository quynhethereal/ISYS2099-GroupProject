...
Product.findBySellerId = async (params) => {
    try {
        ...
        const res = await new Promise((resolve, reject) => {
            seller_pool.execute(
                "SELECT p.title, p.price, p.image, p.description, i.quantity, i.reserved_quantity, p.height, p.width, p.length, p.created_at, p.updated_at FROM `products` p join `inventory` i on i.product_id = p.id WHERE p.seller_id = ? ORDER BY p.id ASC LIMIT ?,?",
